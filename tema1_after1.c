/* Copyright_2019_Paune_Lucian_Dragos_313CA_Tema1 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (p != NULL) {
            ...
            choose_command2(com, playlist, write);
        }
...
void remove_nth_node(struct DoublyLinkedList *list, int n) {
    // printf("APELARE REMOVE!!!\n");
    if (list == NULL || n < 0 || list->size == 0) {
        // return NULL;
        return;
    }
    
    struct Node *aux, *trash;
...
void choose_command2(char *p, struct DoublyLinkedList *list, FILE *write) {
    int pos = 0, i;
    struct Node *aux;
    ...
    if (strcmp(p, "SHOW_PLAYLIST\n") == 0) {
        ...
        for (i = 0; i < list->size; i++) {
            ...
            fprintf(write, "%s", aux->data->title);
            if (i != list->size - 1) {
                // printf("; ");
                fprintf(write, "; ");
            }
